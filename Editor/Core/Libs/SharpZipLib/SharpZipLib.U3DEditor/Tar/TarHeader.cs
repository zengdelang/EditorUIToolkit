...
namespace SharpZipLib.U3DEditor.Tar
{
	...
	public class TarHeader : ICloneable
	{
		...
		/// <summary>
		/// The length of the m_Size field in a header buffer.
		/// </summary>
		...
		/// <summary>
		/// Get/set the entry's m_Size.
		/// </summary>
		/// <exception cref="ArgumentOutOfRangeException">Thrown when setting the m_Size to less than zero.</exception>
		public long Size {
			get { return size; }
			set {
				if (value < 0) {
		...
		static public StringBuilder ParseName(byte[] header, int offset, int length)
		{
			...
			if (offset + length > header.Length) {
				throw new ArgumentException("Exceeds header m_Size", "length");
			}
