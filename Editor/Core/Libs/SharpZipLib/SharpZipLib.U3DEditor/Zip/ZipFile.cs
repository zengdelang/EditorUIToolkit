...
namespace SharpZipLib.U3DEditor.Zip
{
	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		long TestLocalHeader(ZipEntry entry, HeaderTest tests)
		{
			lock (baseStream_) {
				...
				if (localExtraData.Find(1)) {
					...
					// and m_Size or compressedSize = MaxValue, due to rogue creators.
					...
					if ((localFlags & (int)GeneralBitFlags.Descriptor) != 0) {
						...
						if ((compressedSize != -1) && (compressedSize != entry.CompressedSize)) {
							throw new ZipException("Compressed m_Size invalid for descriptor");
						}
					}
				} else {
				...
				if (testHeader) {
					...
					// This will also apply to streamed entries where m_Size isnt known and the header cant be patched
					...
					// Directories have zero actual m_Size but can have compressed m_Size
					if (entry.IsDirectory) {
						if (size > 0) {
							throw new ZipException("Directory cannot have m_Size");
						}

						// There may be other cases where the compressed m_Size can be greater than this?
						// If so until details are known we will be strict.
						if (entry.IsCrypted) {
							if (compressedSize > ZipConstants.CryptoHeaderSize + 2) {
								throw new ZipException("Directory compressed m_Size invalid");
							}
						} else if (compressedSize > 2) {
							// When not compressed the directory m_Size can validly be 2 bytes
							// if the true m_Size wasnt known when data was originally being written.
				...
				if (((localFlags & (int)GeneralBitFlags.Descriptor) == 0) ||
					...
					if ((compressedSize != 0)
						...
						throw new ZipException(
							string.Format("Compressed m_Size mismatch between central header({0}) and local header({1})",
		...
		/// <summary>
		/// Get /set the buffer m_Size to be used when updating this zip file.
		/// </summary>
		...
		*/
		...
		void WriteLocalEntryHeader(ZipUpdate update)
		{
			...
			if (entry.LocalHeaderRequiresZip64) {
				...
				// Local entry header always includes m_Size and compressed m_Size.
		...
		int WriteCentralDirectoryHeader(ZipEntry entry)
		{
			...
			if (entry.Size < 0) {
				throw new ZipException("Attempt to write central directory entry with unknown m_Size");
			}
			...
			long bytesToCopy, bool updateCrc)
		{
			...
			// NOTE: Compressed m_Size is updated elsewhere.
			var crc = new Crc32();
			byte[] buffer = GetBuffer();
		...
		/// <summary>
		/// Get the m_Size of the source descriptor for a <see cref="ZipUpdate"/>.
		/// </summary>
		/// <param name="update">The update to get the m_Size for.</param>
		/// <returns>The descriptor m_Size, zero if there isnt one.</returns>
				...
				using (source) {
					...
					if (update.OutEntry.Size < 0) {
						...
						if (update.OutEntry.Size != sourceStreamLength) {
							throw new ZipException("Entry m_Size/stream m_Size mismatch");
						}
					}
		...
		void CopyEntryDirect(ZipFile workFile, ZipUpdate update, ref long destinationPosition)
		{
			...
			if (skipOver) {
				if (update.OffsetBasedSize != -1)
					...
					// "caution: value of lrec.csize (compressed m_Size) changed from ..."
					destinationPosition +=
						(sourcePosition - entryDataOffset) + NameLengthOffset + // Header m_Size
		...
		void RunUpdates()
		{
			...
			try {
				...
				foreach (ZipUpdate update in updates_) {
					if (update != null) {
						// If the m_Size of the entry is zero leave the crc as 0 as well.
		...
		class ZipUpdate
		{
			...
			/// <summary>
			/// Get/set the location of the m_Size patch for this update.
			/// </summary>
			...
			/// <summary>
			/// Get/set the m_Size calculated by offset.
		...
		void ReadEntries()
		{
			...
			// 
			// The search is limited to 64K which is the maximum m_Size of a trailing comment field to aid speed.
			...
			if ((thisDiskNumber == 0xffff) ||
				...
				// NOTE: Record m_Size = SizeOfFixedFields + SizeOfVariableData - 12.
				...
				// NOTE: zip64 extensible data sector (variable m_Size) is ignored.
			}
		...
		// Hint: Set always ZipEntry m_Size before they are added to an archive and this setting isnt needed.
		...
		class PartialInputStream : Stream
		{
			...
			public PartialInputStream(ZipFile zipFile, long start, long length)
			{
				...
				// on the m_Size of the files and when garbage is collected.
				//
