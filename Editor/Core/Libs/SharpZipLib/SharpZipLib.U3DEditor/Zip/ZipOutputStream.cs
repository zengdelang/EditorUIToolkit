...
namespace SharpZipLib.U3DEditor.Zip
{
	...
	/// m_Size, compressed m_Size, CRC, etc.
	/// 
	...
	public class ZipOutputStream : DeflaterOutputStream
	{
		...
		public void PutNextEntry(ZipEntry entry)
		{
			...
			if (headerInfoAvailable == false) {
				if (CanPatchEntries == false) {
					// Only way to record m_Size and compressed m_Size is to append a data descriptor
			...
			if (headerInfoAvailable) {
				...
				if (entry.LocalHeaderRequiresZip64 || patchEntryHeader) {
					...
					WriteLeInt(0);  // Compressed m_Size
					WriteLeInt(0);  // Uncompressed m_Size
				}
			}
		...
		public void CloseEntry()
		{
			...
			if (curEntry.Size < 0) {
				...
				throw new ZipException("m_Size was " + size + ", but I expected " + curEntry.Size);
			}

			if (curEntry.CompressedSize < 0) {
				curEntry.CompressedSize = csize;
			} else if (curEntry.CompressedSize != csize) {
				throw new ZipException("compressed m_Size was " + csize + ", but I expected " + curEntry.CompressedSize);
			}
		...
		private static void AddExtraDataAES(ZipEntry entry, ZipExtraData extraData)
		{
			...
			//extraData.AddLeShort(7);							// Data m_Size (currently 7)
		...
		private void WriteAESHeader(ZipEntry entry)
		{
			...
			//
			// Value in the "compressed m_Size" fields of the local file header and the central directory entry
			// is the total m_Size of all the items listed above. In other words, it is the total m_Size of the
		...
		/// <exception cref="ZipException">Archive m_Size is invalid</exception>
		...
		/// <summary>
		/// Used to track the m_Size of data for an entry during writing.
		/// </summary>
		long size;
		...
		// NOTE: Setting the m_Size for entries before they are added is the best solution!
