...
namespace SharpZipLib.U3DEditor.Zip
{
	...
	/// 				    	while (true) {
	/// 				    		m_Size = s.Read(data, 0, data.Length);
	/// 				    		if (m_Size > 0) {
	/// 				    			Console.Write(new ASCIIEncoding().GetString(data, 0, m_Size));
	/// 				    		} else {
	/// 				    			break;
	/// 				    		}
	/// 				    	}
	...
	public class ZipInputStream : InflaterInputStream
	{
		...
		/// <summary>
		/// Returns the current m_Size that can be read from the current entry if available
		/// </summary>
		/// <exception cref="ZipException">Thrown if the entry m_Size is not known.</exception>
		...
		int InitialRead(byte[] destination, int offset, int count)
		{
			...
			if (entry.IsCrypted) {
				...
				if (csize >= ZipConstants.CryptoHeaderSize) {
					...
					throw new ZipException(string.Format("Entry compressed m_Size {0} too small for encryption", csize));
				}
			} else {
		...
		int BodyRead(byte[] buffer, int offset, int count)
		{
			...
			if (offset + count > buffer.Length) {
				throw new ArgumentException("Offset + count exceeds buffer m_Size");
			}
