...
namespace SharpZipLib.U3DEditor.Zip
{
	...
	public class ZipEntry : ICloneable
	{
		...
		/// <br/>
		/// Bit 3: If set, the fields crc-32, compressed m_Size
		/// and uncompressed m_Size are were not able to be written during zip file creation
		...
		/// <summary>
		/// Gets/Sets the m_Size of the uncompressed data.
		/// </summary>
		/// <returns>
		/// The m_Size or -1 if unknown.
		/// </returns>
		/// <remarks>Setting the m_Size before adding an entry to an archive can help
		...
		/// <summary>
		/// Gets/Sets the m_Size of the compressed data.
		/// </summary>
		/// <returns>
		/// The compressed entry m_Size or -1 if unknown.
		/// </returns>
		...
		internal int AESSaltLen {
			get {
				// Key m_Size -> Salt length: 128 bits = 8 bytes, 192 bits = 12 bytes, 256 bits = 16 bytes.
		...
		internal void ProcessExtraData(bool localHeader)
		{
			...
			if (extraData.Find(0x0001)) {
				...
				// The recorded m_Size will change but remember that this is zip64.
				forceZip64_ = true;
				...
				// ...
				//	4.4.8 compressed m_Size: (4 bytes)
				//	4.4.9 uncompressed m_Size: (4 bytes)
				// 
				//		The m_Size of the file compressed (4.4.8) and uncompressed,
				//		(4.4.9) respectively.  When a decryption header is present it 
				//		will be placed in front of the file data and the value of the
				//		compressed file m_Size will include the bytes of the decryption
				//		header.  If bit 3 of the general purpose bit flag is set, 
				//		these fields are set to zero in the local header and the 
				//		correct values are put in the data descriptor and
				//		in the central directory.  If an archive is in ZIP64 format
				//		and the value in this field is 0xFFFFFFFF, the m_Size will be
				//		in the corresponding 8 byte ZIP64 extended information 
				//		extra field.  When encrypting the central directory, if the
				//		local header is not in ZIP64 format and general purpose bit 
				//		flag 13 is set indicating masking, the value stored for the 
				//		uncompressed m_Size in the Local Header will be zero. 
				// 
		...
		private void ProcessAESExtraData(ZipExtraData extraData)
		{
			...
			if (extraData.Find(0x9901)) {
				...
				int length = extraData.ValueLength;         // Data m_Size currently 7
				if (length < 7)
